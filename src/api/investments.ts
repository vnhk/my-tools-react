...
export const walletsApi = {
  ...
  getById: (id: string) => client.get<Wallet>(`/invest-track/wallets/${id}`),
  ...
  delete: (id: string) => client.delete(`/invest-track/wallets/${id}`),

  createSnapshot: (walletId: string, data: Partial<WalletSnapshot>) =>
    client.post<WalletSnapshot>(
      `/invest-track/wallets/${walletId}/snapshots`,
      data
    ),
  ...
  updateSnapshot: (
    walletId: string,
    snapshotId: string,
    data: Partial<WalletSnapshot>
  ) =>
    client.put<WalletSnapshot>(
      `/invest-track/wallets/${walletId}/snapshots/${snapshotId}`,
      data
    ),
...
export const budgetEntriesApi = {
  ...
  delete: (id: string) => client.delete(`/invest-track/budget-entries/${id}`),

  scanReceipt: (base64Image: string, entryDate: string) =>
    client.post<BudgetEntry[]>("/invest-track/budget-entries/scan-receipt", {
      base64Image,
      entryDate,
    }),
};
...
export const realEstateApi = {
  ...
  update: (id: string, data: Partial<RealEstate>) =>
...
export const valuableApi = {
  ...
  update: (id: string, data: Partial<Valuable>) =>
...
export const stockAlertsApi = {
  ...
  delete: (id: string) => client.delete(`/invest-track/stock-alerts/${id}`),
};

export const recommendationsApi = {
  getAll: (params?: Record<string, unknown>) =>
    client.get<Page<InvestmentRecommendation>>(
      "/invest-track/recommendations",
      { params }
    ),

  create: (data: Partial<InvestmentRecommendation>) =>
    client.post<InvestmentRecommendation>(
      "/invest-track/recommendations",
      data
    ),

  update: (id: string, data: Partial<InvestmentRecommendation>) =>
    client.put<InvestmentRecommendation>(
      `/invest-track/recommendations/${id}`,
      data
    ),

  delete: (id: string) => client.delete(`/invest-track/recommendations/${id}`),
};
...
export const stockReportApi = {
  ...
  getReport: (date: string, strategy: string) =>
    client.get<StockReportDto>("/invest-track/stock-report", {
      params: { date, strategy },
    }),

  triggerMorning: () =>
    client.post<{ message: string }>(
      "/invest-track/stock-report/trigger/morning"
    ),

  triggerEvening: () =>
    client.post<{ message: string }>(
      "/invest-track/stock-report/trigger/evening"
    ),
};
