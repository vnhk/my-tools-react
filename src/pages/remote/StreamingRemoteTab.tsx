...
import type {RemoteStatus} from '../../common/hooks/useRemoteControl'
import {fetchProductionDetails, fetchProductions} from '../streaming-platform/api'
import type {ProductionDetails, ProductionSummary} from '../streaming-platform/types'
...
export function StreamingRemoteTab({connected, send, status}: Props) {
        ...
        <>
            ...
            <div className={styles.controls}>
                ...
                {!!status?.audioTracks?.length && (
                    <div className={styles.section}>
                        <div className={styles.searchLabel}>Audio</div>
                        <div className={styles.filterChips}>
                            {status.audioTracks.map((t) => (
                                <button
                                    key={t.index}
                                    className={
                                        status.activeAudioTrack === t.index
                                            ? styles.chipActive
                                            : styles.chip
                                    }
                                    disabled={!connected}
                                    onClick={() => send('AUDIO_TRACK', {index: t.index})}
                                >
                                    {t.name || t.lang || `Track ${t.index + 1}`}
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {!!status?.subtitleLangs?.length && (
                    <div className={styles.section}>
                        <div className={styles.searchLabel}>Subtitles</div>
                        <div className={styles.filterChips}>
                            <button
                                className={
                                    !status.activeSubtitle
                                        ? styles.chipActive
                                        : styles.chip
                                }
                                disabled={!connected}
                                onClick={() => send('SUBTITLE_TRACK', {lang: null})}
                            >
                                Off
                            </button>
                            {status.subtitleLangs.map((lang) => (
                                <button
                                    key={lang}
                                    className={
                                        status.activeSubtitle === lang
                                            ? styles.chipActive
                                            : styles.chip
                                    }
                                    disabled={!connected}
                                    onClick={() => send('SUBTITLE_TRACK', {lang})}
                                >
                                    {lang.toUpperCase()}
                                </button>
                            ))}
