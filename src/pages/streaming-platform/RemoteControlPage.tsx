...
export default function RemoteControlPage() {
  ...
  useEffect(() => {
    ...
    fetchProductions()
      .then((res) => { if (mounted) setProductions(res.data) })
      .catch(() => { /* ignore */ })
  ...
  const playFirstAvailable = async (name: string) => {
    ...
    try {
      ...
      if (videoId) {
        navigateOnTv(`/streaming/player/${encodeURIComponent(name)}/${videoId}`)
      } else {
        openProduction(name)
      }
    } catch {
  ...
  const playFirstAvailable = async (name: string) => {
    ...
    try {
      ...
      if (videoId) {
        ...
        // Fallback: open production page when no video found
        openProduction(name)
      }
    } catch {
      openProduction(name)
    ...
    <div className={styles.page}>
      ...
      {!connectedRoomId ? (
        ...
        <>
          ...
          {/* Search and quick play */}
          <div className={styles.searchSection}>
            <label className={styles.searchLabel}>Search to play</label>
            <input
              type="search"
              className={styles.searchInput}
              placeholder={loadingProds ? 'Loading…' : 'Search movie or series…'}
              value={query}
            ...
            {normalizedQuery && (
              <div className={styles.results}>
                ...
                {filtered.map((p) => (
                  <div key={p.productionName} className={styles.resultItem}>
                    <div className={styles.resultInfo}>
                      <div className={styles.resultTitle}>{p.title ?? p.productionName}</div>
                      <div className={styles.resultMeta}>
                        {p.type?.toLowerCase() ?? 'unknown'} · {p.releaseYearStart ?? ''}
                      </div>
                    </div>
                    <div className={styles.resultActions}>
                      <button
                        ...
                        disabled={!connected}
                        onClick={() => openProduction(p.productionName)}
                      ...
                      <button
                        ...
                        disabled={!connected}
                        onClick={() => void playFirstAvailable(p.productionName)}
