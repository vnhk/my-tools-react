...
function MacroTile({label, consumed, target, unit = 'g'}: {
    ...
    return (
        <div className={styles.macroTile}>
            ...
            <div className={styles.macroConsumed}>{consumed.toFixed(unit === 'kcal' ? 0 : 1)}<span
                className={styles.macroUnit}>{unit}</span></div>
            {target != null && (
                <>
                    ...
                    <div className={styles.macroBar}>
                        <div className={`${styles.macroBarFill} ${over ? styles.overBar : ''}`}
                             style={{width: `${pct}%`}}/>
                    </div>
...
function AddItemDialog({date, mealType, mealName, onAdded, onClose}: {
    ...
    const handleAdd = () => {
        if (tab === 'ingredient') {
            ...
            const g = parseFloat(amountGrams)
            if (isNaN(g) || g <= 0) {
                showError('Enter amount in grams');
                return
            }
            ...
            <div className={styles.dialog} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.tabBar}>
                    <button className={`${styles.tabBtn} ${tab === 'ingredient' ? styles.tabActive : ''}`}
                            onClick={() => setTab('ingredient')}>From Ingredient
                    </button>
                    <button className={`${styles.tabBtn} ${tab === 'quick' ? styles.tabActive : ''}`}
                            onClick={() => setTab('quick')}>Quick Entry
                    </button>
                </div>
                        ...
                        <>
                                        ...
                                        <div className={styles.suggestions}>
                                            {suggestions.map(s => (
                                                <div key={s.id} className={styles.suggestion}
                                                    ...
                                                    {s.icon} {s.name}
                                                    {s.kcalPer100g != null && <span
                                                        className={styles.kcalHint}>{s.kcalPer100g} kcal/100g</span>}
                                                </div>
                                            ))}
                                        </div>
                                    )}
                            ...
                            {selectedIng && (
                                <div className={styles.ingPreview}>
                                    ...
                                    {selectedIng.kcalPer100g != null && <span
                                        className={styles.muted}>{selectedIng.kcalPer100g} kcal / {selectedIng.proteinPer100g}P / {selectedIng.fatPer100g}F / {selectedIng.carbsPer100g}C per 100g</span>}
                                </div>
                            )}
                            <div className={styles.field}>
                        ...
                        <>
                            <div className={styles.tabBar} style={{marginBottom: 0}}>
                                <button className={`${styles.tabBtn} ${quickMode === 'direct' ? styles.tabActive : ''}`}
                                        onClick={() => setQuickMode('direct')}>Direct
                                </button>
                                <button
                                    className={`${styles.tabBtn} ${quickMode === 'per100g' ? styles.tabActive : ''}`}
                                    onClick={() => setQuickMode('per100g')}>Per 100g
                                </button>
                            </div>
                            <div className={styles.field}>
                                <label className={styles.label}>Description</label>
                                <input className={styles.input} value={desc}
                                       onChange={e => setDesc(e.target.value)} placeholder="e.g. Chicken breast"
                                       autoFocus/>
                            </div>
                            {quickMode === 'per100g' && (
                                <div className={styles.field}>
                                ...
                                {[['kcal', kcal, setKcal], ['Protein', protein, setProtein],
                                    ...
                                    ([lbl, val, setter]) => (
                                        <div key={lbl as string} className={styles.field}>
                                            <label
                                                className={styles.label}>{lbl as string}{quickMode === 'per100g' ? '/100g' : ''}</label>
            ...
            <div className={styles.dialog} style={{maxWidth: 360}} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.dialogBody}>
                    <div className={styles.field}>
                        ...
                        <input className={styles.input} type="date" value={srcDate}
...
function SetDataDialog({day, onSaved, onClose}: {
    ...
    const calcTDEE = () => {
        ...
        if (!age || !height || !weight) {
            showError('Enter weight, height and age first');
            return
        }
    ...
    const applyMacroPct = () => {
        ...
        if (!effective) {
            showError('Enter Target Calories first');
            return
        }
        setF(prev => ({
            ...prev,
        ...
        <Dialog open title={`Daily Data — ${day.date}`} onClose={onClose} onConfirm={handleSave}
            ...
            <div className={styles.sdSection}>
                ...
                <div className={styles.sdActivityRow}>
                    ...
                    <div className={styles.sdInfoIcon}
                         title="Fitness trackers often overestimate calorie burn. Select what % of reported activity calories should count toward your daily deficit.">
                        ℹ
                    </div>
                </div>
            </div>
            ...
            <div className={styles.sdSection}>
                ...
                <Button variant="secondary" size="sm" onClick={applyMacroPct} style={{width: '100%'}}>
                    ...
                    (effective: {Number(f.targetKcal || 0) + (Number(f.activityKcal || 0) * Number(f.activityKcalPercent || 100) / 100)} kcal)
                </Button>
            </div>
                    ...
                    <div className={styles.sdField}>
                        ...
                        <div className={styles.sdRadioGroup}>
                            {[{v: 'M', l: '♂ Male'}, {v: 'F', l: '♀ Female'}].map(({v, l}) => (
                                <label key={v}
                                       className={`${styles.sdRadio} ${f.gender === v ? styles.sdRadioActive : ''}`}>
...
export function DietPage() {
    ...
    return (
        <div className={styles.page}>
            ...
            {showSetData && day && (
                <SetDataDialog day={day} onSaved={d => {
                    setDay(d);
